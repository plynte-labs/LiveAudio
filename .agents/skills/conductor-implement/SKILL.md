...
- Commit policy: any instruction in this skill to stage, commit, archive, delete, or revert changes requires explicit user approval first. If approval is not already present in the current user request, stop and ask.
- Tool mapping: use OpenCode `question` for `ask_user`, `apply_patch` for file edits, `bash` for shell commands, and treat Plan Mode instructions as procedural guidance when no plan-mode tool exists.
...
2.  **Locate and Parse Tracks Registry:**
    ...
    -   Read and parse this file. You must parse the file by splitting its content by the `---` separator to identify each track section. For each section, extract the status (`[ ]`, `[~]`, `[x]`), the track description from either `## [ ] Track: <Description>` or `- [ ] **Track: <Description>**`, and the link to the track folder when present.
...
2.  **Update Status to 'In Progress':**
    ...
    -   This requires finding the specific entry for the track (e.g., `## [ ] Track: <Description>` or `- [ ] **Track: <Description>**`) and replacing it with the updated status (e.g., `[~]`) in the **Tracks Registry** file you identified earlier.
...
5.  **Finalize Track:**
    ...
    -   This requires finding the specific entry for the track (e.g., `## [~] Track: <Description>` or `- [~] **Track: <Description>**`) and replacing it with the completed status (e.g., `[x]`).
    -   **Commit Changes:** Ask for explicit user approval before staging or committing the **Tracks Registry** file. If approved, commit with the message `chore(conductor): Mark track '<track_description>' as complete`.
...
6.  **Final Report:** Announce the completion of the synchronization process and provide a summary of the actions taken.
    ...
    - **Commit Changes:**
        - If any files were changed (**Product Definition**, **Tech Stack**, or **Product Guidelines**), you MUST ask for explicit user approval before staging or committing them.
    ...
    *   **If user chooses "Delete":**
        ...
        ii. **Handle Confirmation:**
            - **If 'yes'**:
                ...
                c. **Commit Changes:** Ask for explicit user approval before staging or committing the **Tracks Registry** file and the deletion of the track directory. If approved, commit with the message `chore(conductor): Delete track '<track_description>'`.
